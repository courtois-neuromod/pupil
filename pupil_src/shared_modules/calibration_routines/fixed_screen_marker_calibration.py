...
from . screen_marker_calibration import *

logger = logging.getLogger(__name__)


class Fixed_Screen_Marker_Calibration(Calibration_Plugin):
    ...
    def init_ui(self):
        super().init_ui()
        self.menu.label = "Fixed Screen Marker Calibration"
    ...
    def recent_events(self, events):
        ...
        if self.active:
            ...
            if on_position and events['pupil']:
                ref = {}
                ref["norm_pos"] = self.active_site
                ref["screen_pos"] = self.active_site
                ref["timestamp"] = events['pupil'][0]['timestamp']
                self.ref_list.append(ref)

            # Always save pupil positions
            self.pupil_list.extend(events['pupil'])

            if on_position and events.get('fixations', []):
                fixation_boost = 5
